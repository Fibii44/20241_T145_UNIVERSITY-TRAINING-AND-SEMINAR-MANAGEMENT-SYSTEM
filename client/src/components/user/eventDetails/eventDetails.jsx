...
import Modal from '../../modals/googleFormModal/formModal'
...
function Event() {
        ...
        const fetchEventAndData = async () => {
            try {
                ...
                if (token) {
                    ...
                    setCanRegister(() => {
                        const participantGroup = eventResponse.data?.participantGroup;
                        console.log(participantGroup);

                        // 1. Check if customParticipants list exists and contains the user's email
                        if (hasCustomParticipants) {
                            return eventResponse.data.customParticipants.includes(decodedToken.email);
                        }
                    
                        // 2. Handle "All" for college (open to all users)
                        if (!participantGroup || participantGroup.college === "All") {
                            return true; // Allow everyone to register
                        }
                    
                        // 3. Check if the college matches and no department is specified
                        if (participantGroup.college && (!participantGroup.department || participantGroup.department === "All")) {
                            console.log(decodedToken.college === participantGroup.college);
                            return decodedToken.college === participantGroup.college;
                        }
                    
                        // 4. Check if both college and department match
                        if (participantGroup.college && participantGroup.department) {
                            return (
                                decodedToken.college === participantGroup.college &&
                                decodedToken.department === participantGroup.department
                            );
                        }
                    
                        // Default to disallow registration
                        return false;
                    });
    ...
    const checkCertificateStatus = async () => {
         try {
            ...
            if (response.data.certificate.status === 'generated') {
                                                ...
                                                {hasSubmittedForm && (
                                                    <div className="certificate-section">
                                                        ...
                                                        {certificateStatus === 'generated' ? (
                                                            <>
                                                                <p>Your certificate is ready! Check your email or certificate section for the certificate.</p>
                                                            </>
